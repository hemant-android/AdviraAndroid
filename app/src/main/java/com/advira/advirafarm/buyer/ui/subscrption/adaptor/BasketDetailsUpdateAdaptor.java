...
public class BasketDetailsUpdateAdaptor extends RecyclerView.Adapter<BasketDetailsUpdateAdaptor.BasketDetailsUpdatViewHolder> implements IConsts {
    ...
    String SelectedDate= SubscriptionDetailsActivity.SelectedDate;

    private List<BasketDatum> productList;

    public BasketDetailsUpdateAdaptor(Context mContext, List<BasketDatum> productList) {
    ...
    @NonNull
    @Override
    public BasketDetailsUpdatViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        LayoutInflater inflater = LayoutInflater.from(mContext);
        View view = inflater.inflate(R.layout.layout_dairybasket, null);
        return new BasketDetailsUpdatViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull BasketDetailsUpdatViewHolder holder, int position) {

        BasketDatum product = productList.get(position);
...
*/
        ...
        holder.tv_pack.setText("1");
        holder.tv_productid.setText(String.valueOf(product.getBasketProductId()));
        holder.tv_minqty.setText("Min order : 1");
        holder.integer_number.setText(product.getpCount());
        holder.tv_packsize.setText(product.getpUnits() + " " + product.getpUnitType());
...
*/        holder.tv_unitid.setText(String.valueOf(product.getProductId()));
    ...
    private void CalculatePrice() {
        ...
        if (productList.size() > 0) {
            ...
            CartActivity.tv_rv1.setText("My Cart " + "(" + productList.size() + ")");*/
            for (int i = 0; i < productList.size(); i++) {

                itemprice = Double.parseDouble(productList.get(i).getpTotalPrice());
                carttotal = carttotal + itemprice;

                itemmrp = Double.parseDouble(productList.get(i).getpPricePerPack());
                mrptotal = mrptotal + itemmrp;

                itemtax = Double.parseDouble(productList.get(i).getpTax().replaceAll("%", ""));
            ...
            amountpayable = carttotal*subscriptionDays;
            ...
            //SubscriptionPreviewActivity.tv_footertotal.setText("₹ " + form.format(amountpayable));
            //SubscriptionPreviewActivity.tv_footertotalitem.setText(productList.size() + " ITEMS");
...
/*
            ...
            {
                SharedPrefUtil.setCartCountB2B(mContext, SHARED_PREF_CARTCOUNTB2B, String.valueOf(productList.size()));
            }
            else
            {
                SharedPrefUtil.setCartCountB2C(mContext, SHARED_PREF_CARTCOUNTB2C, String.valueOf(productList.size()));
            }
            SharedPrefUtil.setCartCount(mContext, SHARED_PREF_CARTCOUNT, String.valueOf(productList.size()));
            try {
                MainActivityNav.text.setText(String.valueOf(productList.size()));
                CartActivity.text.setText(String.valueOf(productList.size()));
            } catch (Exception ex) {
            ...
            try {
                ...
                MainActivityGuestNav.text.setText(String.valueOf(productList.size()));
            ...
            try {
                ...
                ProductDetailsActivityGuest.text.setText(String.valueOf(productList.size()));
            ...
            try {
                ProductDetailsActivity.text.setText(String.valueOf(productList.size()));
            } catch (Exception ex) {
...
*/
    ...
    public class BasketDetailsUpdatViewHolder extends RecyclerView.ViewHolder {
        ...
        public BasketDetailsUpdatViewHolder(@NonNull View itemView) {
            super(itemView);
